...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	uint32_t delay2 = HAL_GetTick();
	uint32_t delay3 = HAL_GetTick();
	...
	while (1)
	{
		...
		if (HAL_GetTick() > delay2 + 300) {
			...
			case TEPLOMER:
				...
				sct_value(ntc_lookup[ADC_value], 0, 2);
			...
			case NTC:
				...
				if (HAL_GetTick() > delay3 + NON_BLOCK_DELAY) {
